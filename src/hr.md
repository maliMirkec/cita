---
layout: default
title: CiTA.hr
sections:
  - title: Mi ćemo napraviti vašu stranicu
    subtitle: Ali uz vašu malu pomoć
    gfx: help.svg
    list:
      - title: Bit će brza
        subtitle: Jer posjetitelji vole brzo učitavanje
        bg: speed.svg
      - title: Bit će optimizirana za SEO
        subtitle: Jer tražilice vole odličan SEO
        bg: search.svg
        type: split
      - title: Bit će pristupačna
        subtitle: Jer svaki posjetitelj mora moći pristupiti sadržaju
        bg: accessible.svg
        type: split
      - title: Izgledat će dobro na svim uređajima
        subtitle: Jer posjetitelji dolaze sa svih strana svijeta
        bg: speed.svg
        type: split
      - title: Bit će unikatna*
        subtitle: Jer ste i vi i vaš posao unikatni
        bg: speed.svg
    type: split
    nav: Usluge
  - title: Vaša stranica može izgledati ovako
    websites:
      - title: Heyflow
        href: https://heyflow.com
      - title: Wunder Mobility
        href: https://wundermobility.com
    type: work
    nav: Projekti
  - title: Ovo su naši zadovoljni klijenti
    clients:
      - title: Heyflow
        href: https://heyflow.com
    type: testimonials
    nav: Preporuke
  - title: Kontaktirajte nas
    clients:
      - title: Heyflow
        href: https://heyflow.com
    type: testimonials
    nav: Kontakt
---

{{ title }}
